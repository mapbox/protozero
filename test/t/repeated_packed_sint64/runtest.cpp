...
TEST_CASE("repeated_packed_sint64") {
    ...
    SECTION("one") {
        ...
        REQUIRE(item.next());
        auto it_pair = item.packed_sint64();
        REQUIRE(!item.next());
    ...
    SECTION("end_of_buffer") {
        ...
        for (size_t i=1; i < buffer.size(); ++i) {
            ...
            REQUIRE(item.next());
            REQUIRE_THROWS_AS(item.packed_sint64(), mapbox::util::pbf::end_of_buffer_exception);
        }
    }

}

TEST_CASE("write repeated_packed_sint64") {
    ...
    SECTION("empty") {
        ...
        pw.add_packed_sint64(1, nullptr, nullptr);
